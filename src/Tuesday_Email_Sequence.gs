...
/************************************************************
 * EMAIL TEMPLATES (Source: Tuesday_HTML_Sequence_Script.js)
 ************************************************************/

function getTuesdayInitialTemplate(name, comp) {
...
function getTuesdayFU1Template(name, comp) {
  return {
...
function getTuesdayFU2Template(name, comp) {
  return {
...
function getTuesdayFU3Template(name, comp) {
  return {
...
/************************************************************
 * PROFESSIONAL FOOTER (Source: Tuesday_HTML_Sequence_Script.js)
 ************************************************************/

function getTuesdayProfessionalFooter() {
  return `
    <br><br>
...
/************************************************************
 * MAIN SEQUENCE (Logic from Gpt_Correct mail_approach.js)
 ************************************************************/

function sendTuesdayHTMLSequenceEmails() {
...
function processTuesdayEmail(
  ...
  if (
    ...
    const template =
      getTuesdayInitialTemplate(name, comp);
    ...
    // We search by recipient and timestamp.
    // Searching by the exact complex subject often fails due to Gmail's indexing of special characters.
    const query =
      'in:sent to:' +
      cleanEmail +
      ' after:' +
    ...
    /*
     * Gmail indexing can sometimes take a few seconds.
    ...
    for (let attempt = 1; attempt <= 5; attempt++) {
      ...
      try {
        ...
        for (let j = 0; j < messages.length; j++) {
          ...
          // Use 'includes' instead of '===' to handle slight variations in how Gmail stores subjects
  ...
  if (
    ...
    sendTuesdayFollowUp(
      ...
      threadId,
      messageId,
      getTuesdayFU1Template(name, comp),
      "FU1 Sent"
    );
  ...
  if (
    ...
    sendTuesdayFollowUp(
      ...
      threadId,
      messageId,
      getTuesdayFU2Template(name, comp),
      "FU2 Sent"
    );
  ...
  if (
    ...
    sendTuesdayFollowUp(
      ...
      threadId,
      messageId,
      getTuesdayFU3Template(name, comp),
      "Completed"
    );
...
function sendTuesdayFollowUp(
   ...
   * To ensure Gmail threads the email, the subject must be
   * exactly "Re: " + Initial Subject.
   **********************************************************/
  const initialTemplate = getTuesdayInitialTemplate(name, comp);
  const replySubject = "Re: " + initialTemplate.subject;


  const finalBody =
    template.body +
    getTuesdayProfessionalFooter();
   ...
   *
   * DO NOT FALL BACK TO GmailApp.sendEmail()
   *
   * GmailApp.sendEmail() would create a new standalone email.
   **********************************************************/
   ...
   * IMPORTANT:
   *
   * sentMessage.id = Gmail internal ID
   *
   * It is NOT the same thing as:
   *
   * Message-ID: <xxxxx@mail.gmail.com>
   *
   * We need the RFC Message-ID for the NEXT reply.
   **********************************************************/
   ...
   * UPDATE MESSAGE ID
   *
   * Store RFC Message-ID, NOT Gmail internal ID.
   **********************************************************/
 ...
 * TEST: INITIAL EMAIL
 *
 * Sends initial email for the first data row.
 ************************************************************/

function runTuesdayInitialTest() {
  ...
  if (!email) {
    throw new Error(
      "No email address found in row 2."
    );
  }
